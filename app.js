/* 商品マスターは Google スプレッドシート「Recipe Master」が正。
   起動時に Apps Script(action=products)から取得し localStorage にキャッシュする。
   取得もキャッシュも無いとき(初回インストール直後にオフライン)だけ、
   下の内蔵リストで動く。レジが止まらないことを優先している。 */
const FALLBACK_MENU=[
{id:'SIG12',name:'Signature Matcha Latte',size:'12oz',category:'Matcha',price:99},
{id:'MAN16',name:'Matcha Mango',size:'16oz',category:'Matcha',price:150},
{id:'MAN22',name:'Matcha Mango',size:'22oz',category:'Matcha',price:180},
{id:'CLO16',name:'Matcha Cloud Latte',size:'16oz',category:'Matcha',price:160},
{id:'CLO22',name:'Matcha Cloud Latte',size:'22oz',category:'Matcha',price:190},
{id:'FRA16',name:'Matcha Frappe',size:'16oz',category:'Matcha',price:160},
{id:'FRA22',name:'Matcha Frappe',size:'22oz',category:'Matcha',price:190},
{id:'SEA16',name:'Matcha Sea Salt',size:'16oz',category:'Matcha',price:160},
{id:'SEA22',name:'Matcha Sea Salt',size:'22oz',category:'Matcha',price:190},
{id:'STR16',name:'Matcha Strawberry',size:'16oz',category:'Matcha',price:180},
{id:'STR22',name:'Matcha Strawberry',size:'22oz',category:'Matcha',price:210},
{id:'EIN16',name:'Einspanner Matcha Latte',size:'16oz',category:'Matcha',price:190},
{id:'EIN22',name:'Einspanner Matcha Latte',size:'22oz',category:'Matcha',price:220},
{id:'ORE16',name:'Matcha Oreo',size:'16oz',category:'Matcha',price:210},
{id:'ORE22',name:'Matcha Oreo',size:'22oz',category:'Matcha',price:240},
{id:'NUT16',name:'Nutella Matcha Latte',size:'16oz',category:'Matcha',price:270},
{id:'NUT22',name:'Nutella Matcha Latte',size:'22oz',category:'Matcha',price:300},
{id:'CHU5',name:'Churros',size:'5pcs',category:'Churros',price:110}
];
const ICONS={'Signature Matcha Latte':'🍵','Matcha Cloud Latte':'☁️','Einspanner Matcha Latte':'🥛','Matcha Mango':'🥭','Matcha Strawberry':'🍓','Matcha Oreo':'🍪','Matcha Sea Salt':'🧂','Nutella Matcha Latte':'🍫','Matcha Frappe':'🥤','Churros':'🥖','Milky Oreo':'🍪','Chocolate Milk':'🍫','Strawberry Milk':'🍓','Mango Milky Latte':'🥭','Chocolate Frappe':'🍫','Oreo Frappe':'🍪'};
const CAT_ICON={Matcha:'🍵',Milky:'🥛',Frappe:'🥤',Churros:'🥖'};
const CAT_ORDER=['Matcha','Milky','Frappe','Churros'];
const MENU={rows:[],source:'fallback',fetchedAt:null};
let PRODUCTS=[];
function iconOf(name,cat){return ICONS[name]||CAT_ICON[cat]||'🥤'}
function catRank(c){const i=CAT_ORDER.indexOf(c);return i<0?CAT_ORDER.length:i}
/* MENU.rows(サイズ1行=1件)を、商品名でまとめた PRODUCTS に組み直す */
function rebuildProducts(){
  const m=new Map();
  MENU.rows.forEach(r=>{
    const name=r.name,cat=r.category||'Other';
    if(!m.has(name))m.set(name,{name,category:cat,icon:iconOf(name,cat),sizes:[]});
    m.get(name).sizes.push({size:r.size,price:Number(r.price)||0,id:r.id});
  });
  const list=[...m.values()];
  list.forEach(p=>{p.sizes.sort((a,b)=>a.price-b.price);p.size=p.sizes[0].size;p.price=p.sizes[0].price});
  list.sort((a,b)=>catRank(a.category)-catRank(b.category)||a.price-b.price||a.name.localeCompare(b.name));
  PRODUCTS=list;
  if(state.menuCategory&&!PRODUCTS.some(p=>p.category===state.menuCategory))state.menuCategory=null;
}
function sizesOf(p){return (p&&p.sizes&&p.sizes.length)?p.sizes.map(s=>s.size):[p&&p.size].filter(Boolean)}
function defaultSize(p){return (p&&p.sizes&&p.sizes[0])?p.sizes[0].size:(p&&p.size)||''}
function menuPrice(name,size){
  const p=PRODUCTS.find(x=>x.name===name); if(!p)return 0;
  const hit=p.sizes.find(s=>s.size===size); return hit?hit.price:p.price;
}
/* ₱30割引券の対象判定。
   抹茶カテゴリーの22ozのみ(Signature は集客商品なので除外)。
   非抹茶(Milky / Frappe / Churros)は対象外。 */
function canDiscount(p,size){return !!p&&p.category==='Matcha'&&size==='22oz'&&p.name!=='Signature Matcha Latte'}
function menuCategories(){const seen=[];PRODUCTS.forEach(p=>{if(!seen.includes(p.category))seen.push(p.category)});return seen}
function menuFetchedLabel(){
  if(!MENU.fetchedAt)return 'not yet — using built-in list';
  const d=new Date(MENU.fetchedAt);
  return `${dateKey(d)} ${timeKey(d)}${MENU.source==='cache'?' (saved copy)':''}`;
}
function initMenu(){
  const c=SheetSync.cachedProducts();
  if(c){MENU.rows=c.products;MENU.source='cache';MENU.fetchedAt=c.fetchedAt}
  else{MENU.rows=FALLBACK_MENU.slice();MENU.source='fallback';MENU.fetchedAt=null}
  rebuildProducts();
  refreshMenu(true);
}
async function refreshMenu(silent){
  try{
    const r=await SheetSync.fetchProducts();
    MENU.rows=r.products;MENU.source='sheet';MENU.fetchedAt=r.fetchedAt;
    rebuildProducts();
    if(!silent)alert('Product list updated.');
    if(state.view==='order'||state.view==='recipe')render();
  }catch(e){
...
function buildImportedOrder(raw){
  const items=raw.items.map(it=>{
    const meta=SheetSync.lookupRecipe(it.recipeId)||{name:it.recipeId,size:''};
    const price=menuPrice(meta.name,meta.size);
...
function quickOption(i){
  const p=PRODUCTS[i];
  if(!state.quickOptions[i]) state.quickOptions[i]={size:defaultSize(p),qty:0,discountPct:0};
  return state.quickOptions[i];
}
function quickPrice(p,size){return menuPrice(p.name,size)}
function quickCard(p,i){
  const q=quickOption(i),sizes=p.name==='Signature Matcha Latte'?['12oz']:p.name==='Churros'?['5 sticks']:['16oz','22oz'];
  const price=quickPrice(p,q.size),discount=(canDiscount(p,q.size))?q.discountPct:0,lineTotal=(price-discount)*q.qty;
  return `<div class="card quick-product-card">
    ...
    <div class="quick-controls">
      ...
      <div class="quick-field"><span class="quick-label">Discount</span><div class="quick-chips"><button class="quick-chip ${discount===0?'active':''}" data-quick-discount="${i}" data-discount-value="0">None</button>${(canDiscount(p,q.size))?`<button class="quick-chip ${discount===30?'active discount':''}" data-quick-discount="${i}" data-discount-value="30">₱30 OFF</button>`:''}</div>${!canDiscount(p,q.size)?'<small class="muted">₱30 OFF applies to 22oz matcha drinks only</small>':''}</div>
    </div>
...
function menuTabsHTML(){
  const cats=menuCategories();
  if(cats.length<2)return '';
  return `<div class="menu-tabs" role="tablist">
    <button class="menu-tab ${state.menuCategory?'':'active'}" data-menu-cat="">ALL</button>
    ${cats.map(c=>`<button class="menu-tab ${state.menuCategory===c?'active':''}" data-menu-cat="${c}">${CAT_ICON[c]||''} ${c}</button>`).join('')}
  </div>`;
}
function menuMetaHTML(){
  const warn=MENU.source==='fallback'?'<span class="menu-warn">offline \u2014 built-in list</span>':'';
  return `<div class="menu-meta">
    <small class="muted">Product list last synced: ${menuFetchedLabel()}</small> ${warn}
    <button class="btn menu-refresh-btn" data-menu-refresh>\u{1F504} Refresh product list</button>
  </div>`;
}
function orderView(){
  ...
  ${menuTabsHTML()}
  <div class="product-picker-grid">${PRODUCTS.map((p,i)=>({p,i})).filter(x=>!state.menuCategory||x.p.category===state.menuCategory).map(({p,i})=>{
