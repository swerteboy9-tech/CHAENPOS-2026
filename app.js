...
function homeView(){const t=totals();return `<div class="card hero"><div class="label">TODAY SALES</div><div class="big">${peso(t.sales)}</div><div class="row"><span>${t.cups} items sold</span><span>${dateKey()}</span></div></div><div class="grid2"><button class="btn primary" data-go="order">🧋 ORDER<br><small>Take an order</small></button><button class="btn alt" data-go="expense">🧺 EXPENSE<br><small>Record a cost</small></button><button class="btn" data-go="report">📊 REPORT<br><small>This month</small></button><button class="btn action-red" data-go="close">🔒 CLOSE DAY<br><small>Count the cash</small></button></div><button class="btn btn-wide" data-go="recipe">📖 RECIPES<br><small>What goes in each cup</small></button><div class="card"><div class="section-title">Today’s Summary</div><div class="row"><span>Cash</span><b>${peso(t.cash)}</b></div><div class="row"><span>GCash</span><b>${peso(t.gcash)}</b></div><div class="row"><span>Expenses</span><b>${peso(t.exp)}</b></div><div class="row"><span>Net</span><b>${peso(t.net)}</b></div></div>`}
...
function historySalesDetailHTML(date){const os=orders().filter(o=>o.date===date).sort((a,b)=>a.time.localeCompare(b.time));const total=os.reduce((sum,o)=>sum+Number(o.total||0),0);const itemCount=os.reduce((sum,o)=>sum+o.items.reduce((s,i)=>s+Number(i.qty||0),0),0);return `<button class="btn history-back" data-history-back>← Back to Sales History</button><div class="daily-detail-head card"><div><div class="label">${date}</div><div class="daily-detail-total">${peso(total)}</div></div><div class="right"><b>${os.length} orders</b><br><small>${itemCount} items</small></div></div><div class="list history-list">${os.map((o,idx)=>`<div class="history-order"><div class="history-order-head"><div><b>ORDER ${idx+1} / ${o.time}</b><br><small>${o.payment}</small>${o.backdated?` <span class="history-badge">BACKDATED</span>`:''}${o.editedAt?` <span class="history-badge edited">EDITED</span>`:''}</div><div class="history-order-actions"><div class="history-order-total">${peso(o.total)}</div><button class="mini-edit" data-edit-order="${o.id}">Edit</button><button class="mini-delete" data-delete-order="${o.id}">Delete</button></div></div><div class="history-lines">${o.items.map(i=>`<div class="history-line"><div class="history-icon">${historyProductIcon(i.name)}</div><div class="history-line-name"><b>${i.name}</b><small>${i.size}</small></div><div class="history-qty">× ${i.qty}</div><div class="history-line-total">${peso((i.price-i.discount)*i.qty)}</div></div>`).join('')}</div></div>`).join('')}</div>`}
function historyExpensesHTML(){const es=expenses().slice().reverse().slice(0,30);return es.length?es.map(e=>`<div class="item"><div class="row"><div><b>${e.category}</b><br><small>${e.date} ${e.time} / ${e.payment} ${e.memo?'/ '+e.memo:''}</small></div><div style="text-align:right"><b>-${peso(e.amount)}</b><div class="history-order-actions" style="margin-top:4px"><button class="mini-edit" data-edit-expense="${e.id}">Edit</button><button class="mini-delete" data-delete-expense="${e.id}">Delete</button></div></div></div></div>`).join(''):'<div class="muted">No expense history yet.</div>'}
...
setInterval(()=>{$('#clock').textContent=new Date().toLocaleString('ja-JP',{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'})},1000);render();if('serviceWorker'in navigator)navigator.serviceWorker.register('sw.js');
